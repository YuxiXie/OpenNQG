...
def filter_data(data_list, opt):
    ...
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')  # TODO: fix this
    ...
    for i, src, tgt in zip(idx, data_list[0], data_list[1]):
        ...
        raw_text = ''.join(src).lower()
        bert_text = ''.join([w.lstrip('##') for w in tokenizer.tokenize(' '.join(src))])
        if src_len <= opt.src_seq_length and len(tgt) - 1 <= opt.tgt_seq_length:
            if len(src) * len(tgt) > 0 and src_len >= 10: # and raw_text == bert_text:
                if opt.answer == 'enc':
                    rst[0].append(src)
                else:
...
def convert_word_to_idx(text, vocab, pretrained=False, model='', sep=False):
    ...
    def tokenized_sent(sent):
        sent = sent.split(' ')
        tokens, indexes = [], []
        for word in sent:
            try:
                index = vocab.tokenizer.encode(word)
                indexes += index
                tokens += vocab.convertToLabels(index)
            except:
                indexes += vocab.tokenizer.encode('unk')
                tokens += [word]
        words = vocab.convertToLabels(indexes)
        return (tokens, words)
    ...
    if pretrained:
        ...
        if sep:
            ...
            elif model.count('gpt2'):
                text = [[sent[0].strip().lstrip(Constants.CLS_WORD).strip(), sent[1].strip().rstrip(Constants.SEP_WORD).strip()] for sent in text]
                text = [[tokenized_sent(sent[0]), tokenized_sent(sent[1])] for sent in text]
                tokens = [sent[0][0] + ['<|endoftext|>'] + sent[1][0] + ['<|endoftext|>'] for sent in text]
                words = [sent[0][1] + ['<|endoftext|>'] + sent[1][1] + ['<|endoftext|>'] for sent in text]
                indexes = [vocab.convertToIdx(sent) for sent in words]
                return indexes, tokens
        else:
            if model.count('bert'):
                tokens = [vocab.tokenizer.tokenize(sent) for sent in text]
            elif model.count('gpt2'):
                text = [tokenized_sent(sent.strip().lstrip(Constants.CLS).strip()) for sent in text]
                tokens = [sent[0] for sent in text]
                words = [sent[1] for sent in text]
                indexes = [vocab.convertToIdx(sent) for sent in words]
                return indexes, tokens
...
def main(opt):
    ...
    #========== word to index ==========#
    train_src_idx, train_src_tokens = convert_word_to_idx(train_src, src_vocab, pretrained=opt.bert, 
                                                          model=opt.bert_vocab, sep=sep)
    train_tgt_idx, train_tgt_tokens = convert_word_to_idx(train_tgt, tgt_vocab)
    valid_src_idx, valid_src_tokens = convert_word_to_idx(valid_src, src_vocab, pretrained=opt.bert, 
                                                          model=opt.bert_vocab, sep=sep)
    ...
    if opt.bert_vocab.count('gpt2'):
        data['dict']['src'] = opt.bert_vocab
