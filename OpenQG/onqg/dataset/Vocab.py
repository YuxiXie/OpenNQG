...
def build_vocab(pretrained):
    ...
    def gpt_label2idx(tokens, tokenizer):
        return [tokenizer.encoder[token] for token in tokens]

    def gpt_idx2label(indexes, tokenizer):
        return [tokenizer.decoder[index] for index in indexes]
    ...
    if pretrained.count('bert'):
        ...
        model_file = 'bert'
    elif pretrained.count('gpt2'):
        tokenizer = GPT2Tokenizer.from_pretrained(pretrained)
        labelToIdx = functools.partial(gpt_label2idx, tokenizer=tokenizer)
        idxToLabel = functools.partial(gpt_idx2label, tokenizer=tokenizer)
        model_file = 'gpt2'
    else:
...
class Vocab(object):
    ...
    def from_opt(cls, corpus=None, opt=None, pretrained=None):
        ...
        if pretrained is not None and pretrained.count('gpt2'):
            special_words = ['<|endoftext|>']
            vocab = cls(special_words, lower=False, opts=build_vocab(pretrained))
            return vocab

        special_words = [Constants.PAD_WORD, Constants.UNK_WORD]
        if opt['transf']:
            special_words += [Constants.CLS_WORD]
        if opt['tgt']:
            special_words += [Constants.BOS_WORD, Constants.EOS_WORD]
        if opt['separate']:
            special_words += [Constants.SEP_WORD]
    ...
    def lookup(self, key, default=Constants.UNK):
        try:
            return self.label_to_idx([key])[0] if self.type != 'default' else self.labelToIdx[key]
        except KeyError:
            return default

    def getLabel(self, idx, default=Constants.UNK_WORD):
        try:
            return self.idx_to_label([idx])[0] if self.type != 'default' else self.idxToLabel[idx]
        except KeyError:
            return default
